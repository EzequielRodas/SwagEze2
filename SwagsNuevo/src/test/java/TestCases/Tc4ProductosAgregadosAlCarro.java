...
public class Tc4ProductosAgregadosAlCarro {
	WebDriver driver;
	
	public boolean Test(Datasources listaProductos,Reports report, DriverManager DM,int iteration, String name) {
		...
		try {			
			...
			report.AddLine("Se verifica que esten los: "+listaProductos.getTotalIterations()+" productos agregados en el carro");
			
			result=this.validacion(report,driver,name,repo,listaProductos);
	...
	private boolean validacion(Reports report, WebDriver driver2, String name, Repo_Template repo,
			Datasources listaProductos) {
		boolean validaciones=true;
		
		
		for(int x=0;x<listaProductos.getTotalIterations();x++) {
			validaciones = validaciones&&report.validateObjectIsDisplayable(repo.get_removebtn(listaProductos.getValueFromDataSource("nombreProducto", x)));
		}
		report.Screenshot(name);
		return validaciones;
	}
